...
`python run_checks.py` — control AZ-GOV-001, implemented in
`python/checks/governance.py`. Enumerates every resource group in the
subscription and compares against `MANAGED_RESOURCE_GROUPS` in
...
**Status — pass (remediated 2026-08-15)**

| Resource group | Location | Classification |
|---|---|---|
| NetworkWatcherRG | — | platform-managed — reviewed, out of scope |

`RG-Security-Lab` (northeurope, holding VNET-Security-Lab and NSG-Web-Subnet)
was deleted on 2026-08-15 after the control had been validated against it. It
was a remnant of the initial North Europe attempt blocked by the tenant-root
deny policy `sys.blockwesteurope`, carried no VMs or public IPs, and served no
purpose.

The control was deliberately left failing until the check itself had been proven
in both the local environment and the pipeline. Remediating first would have
removed the only real data available to validate it against.
...
marked as passing, stating why it is out of scope. Silently skipping it would
...
An authorization failure no longer aborts the run. If the identity loses Reader
at subscription scope, this control alone returns `ERROR` and the remaining
controls still execute. `ERROR` blocks the build in the same way `FAIL` does —
a check that could not run has proven nothing, and treating it as a pass would
make the report worse than useless.